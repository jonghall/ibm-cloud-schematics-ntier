...
ibmcloud is instance-create $vsiname $vpc $region $profile $subnet --key-ids $key --image-id $image --security-group-ids $securitygroup --user-data @RaxakProtectSetup.sh 2>1 | tee "temp.txt"
NICID=$(cat temp.txt | awk '/Primary interface/ {print $3}' | sed 's/.*(\(.*\))/\1/')
